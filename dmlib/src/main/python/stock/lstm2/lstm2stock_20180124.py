...
# 获取训练集
def get_train_data(input_size, batch_size, time_step, train_begin, train_end, ylabel):
    batch_index = []
    ...
    for i in range(len(normalized_train_data) - time_step):
        ...
        y = normalized_train_data[i:i + time_step, ylabel, np.newaxis]
...
# 获取测试集
def get_test_data(input_size, time_step, test_begin, ylabel):
...
# ——————————————————训练模型——————————————————
def train_lstm(input_size, batch_size, time_step, train_begin, train_end, iter, ylabel):
    ...
    batch_index, train_x, train_y = get_train_data(input_size, batch_size, time_step,
                                                   train_begin, train_end, ylabel)
    pred, _ = lstm(X)
    # 损失函数
...
# ————————————————预测模型————————————————————
def prediction(input_size, time_step, test_begin, ylabel):
    X = tf.placeholder(tf.float32, shape=[None, time_step, input_size])
    # Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])
    mean, std, test_x, test_y = get_test_data(input_size, time_step, test_begin, ylabel)
    pred, _ = lstm(X)
    ...
    with tf.Session() as sess:
        ...
        test_y = np.array(test_y) * std[ylabel] + mean[ylabel]
        test_predict = np.array(test_predict) * std[ylabel] + mean[ylabel]
...
# ——————————————————导入数据——————————————————————
f = open('data/stock/000977.SZ.csv')
df = pd.read_csv(f)  # 读入股票数据
data = df.iloc[:, 1:7].values  # 取第3-10列
